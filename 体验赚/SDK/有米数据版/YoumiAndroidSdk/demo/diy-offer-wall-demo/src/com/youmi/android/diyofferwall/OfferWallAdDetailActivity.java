...
import com.youmi.android.diyofferwall.adapter.GridViewAdapter;
import com.youmi.android.diyofferwall.adapter.ListViewAdapter_TaskDesc;
import com.youmi.android.diyofferwall.listener.BitmapDownloadListener;
import com.youmi.android.diyofferwall.model.TaskDescObject;
...
import cde.ewd.adw.os.df.AppDetailDataInterface;
import cde.ewd.adw.os.df.AppDetailObject;
	...
	private void updateView(final AppDetailObject detailData) {
		if (detailData != null) {
			...
			for (int i = 0; i < appDetailObject.getScreenShotUrls().length; i++) {
				bmLists.add(BitmapFactory.decodeResource(getResources(), R.drawable.icon));
			}
